...
const CATEGORY_COLORS = {
    "Clutter": [128, 128, 128], "Cormorants": [30, 144, 255], "Pigeons": [138, 43, 226],
    "Ducks": [50, 205, 50], "Geese": [255, 140, 0], "Gulls": [224, 255, 255],
    "Birds of Prey": [220, 20, 60], "Waders": [255, 105, 180], "Songbirds": [255, 215, 0],
    "Unknown": [255, 255, 255],
    "Test Set": [80, 93, 111]
};
...
let TEST_SET_LOADED = false, PENDING_TEST_ACTION = null;
...
function parseWkb(bufferOrArray) {
    ...
    const view = new DataView(bufferOrArray.buffer ? bufferOrArray.buffer : bufferOrArray);
    let offset = bufferOrArray.byteOffset || 0;

    const byteOrder = view.getUint8(offset); offset += 1; // 1 = LE
...
function parseBinaryData(buffer, isTest) {
    ...
    while (offset < buffer.byteLength) {
        // 1. Track ID (Uint32)
        const tid = view.getUint32(offset, true); offset += 4;

        // 2. Timestamps
        const t_start = view.getUint32(offset, true); offset += 4;
        const t_end = view.getUint32(offset, true); offset += 4;

        const strStart = formatTime(t_start);
        if (strStart) days.add(strStart.split(" ")[0]);

        // 3. Trajectory Blob
        const trajLen = view.getUint16(offset, true); offset += 2;
        ...
        const timeLen = view.getUint16(offset, true); offset += 2;
        const times = [];
        for (let i = 0; i < timeLen; i++) {
            times.push(view.getFloat32(offset, true)); offset += 4;
        }

        // 5. Radar Bird Size (Enum)
        const bsIdx = view.getUint8(offset); offset += 1;
        const birdSize = RADAR_SIZES[bsIdx] || "Unknown";

        // 6. Scalars
        const airspeed = view.getFloat32(offset, true); offset += 4;
        const min_z = view.getFloat32(offset, true); offset += 4;
        const max_z = view.getFloat32(offset, true); offset += 4;
        ...
        if (!isTest) {
            // Train only fields
            const obsId = view.getUint32(offset, true); offset += 4;
            const pObsId = view.getUint32(offset, true); offset += 4;

            const posLen = view.getUint16(offset, true); offset += 2;
            offset += posLen; // Skip obs pos WKB for now (not used in UI yet)

            const comLen = view.getUint16(offset, true); offset += 2;
            const comment = dec.decode(buffer.slice(offset, offset + comLen));
            offset += comLen;

            const nBirds = view.getUint16(offset, true); offset += 2;

            const bgIdx = view.getUint8(offset); offset += 1;
            group = BIRD_GROUPS[bgIdx] || "Unknown";

            const spIdx = view.getUint8(offset); offset += 1;
...
async function init() {
    try {
        ...
        document.getElementById('search-class').innerHTML = '<option value="">All Classes</option>' + cleanGroups.map(g => `<option value="${g}">${g}</option>`).join('');
        document.getElementById('search-day').innerHTML = '<option value="">All Days</option>';
...
async function runPythonFilter() {
    ...
    try {
        ...
        for (let i = 0; i < processSource.length; i += CHUNK_SIZE) {
            ...
            results.forEach((keep, idx) => { if (keep) allowedIds.add(chunk[idx].id); });
        }
...
function checkTermsAndLoadTest(callback) {
    if (TEST_SET_LOADED) { if (callback) callback(); return; }
...
async function loadTestSet(callback) {
    ...
    document.getElementById('loader-text').innerText = "Loading & Decrypting Test Set...";
    try {
        ...
        header: true, skipEmptyLines: true, complete: (results) => {
            ...
            results.data.forEach(row => {
                ...
                rows.push({
                    ...
                    coords: coords, times: times, calculated: {}, meta: meta
                });
            });
    ...
    if (type === 'data') {
        ...
        reader.onload = (e) => {
            if (file.name.endsWith('.bin')) {
                ...
                parseCSVContent(e.target.result, document.getElementById('dataset-select').value);
            }
        };
    } else {
        reader.onload = (e) => checkTermsAndLoadTest(() => parseSubmission(e.target.result));
    }
    ...
    Papa.parse(text, {
        header: true, skipEmptyLines: true, complete: (results) => {
            ...
            results.data.forEach(row => {
                ...
                if (!tid) return;
                const scores = {};
                BIRD_GROUPS.forEach(g => { if (g !== "Unknown") scores[g] = parseFloat(row[g]) || 0; });
                const debugVals = {};
...
function applySimpleFilters() {
    ...
    ACTIVE_DATA = RAW_DATA[ds].filter(d => {
        ...
        if (sday && d.meta.timestamp_start_radar_utc && !d.meta.timestamp_start_radar_utc.startsWith(sday)) return false;
...
function sortData(col, toggle = true) {
    ...
    ACTIVE_DATA.sort((a, b) => {
        ...
        else if (col === 'group') { va = a.predictedGroup || a.group; vb = b.predictedGroup || b.group; }
...
function getPathLayer() {
    return new PathLayer({
        ...
        getWidth: d => d.id === SELECTED_ID ? 8 : 3, rounded: true,
        onClick: i => { if (i.object) selectTrack(i.object, true); },
        onHover: i => {
        ...
        updateTriggers: { getColor: [SELECTED_ID, ACTIVE_DATA], getWidth: [SELECTED_ID] }
    });
}
...
function selectTrack(track, openPanel = true) {
    ...
    if (SUBMISSION[track.id]) {
        ...
        predArea.innerHTML = html; predArea.classList.remove('hidden');
    } else predArea.classList.add('hidden');

    const exactSpecies = track.meta.bird_species && track.meta.bird_species !== "Unknown" ? ` | ${track.meta.bird_species}` : "";
    document.getElementById('detail-meta').innerHTML = `Truth: <b class="text-white">${track.group}${exactSpecies}</b>`;
    ...
    setTimeout(() => { const p = document.getElementById('detail-plot'); if (p) Plotly.Plots.resize(p); }, 50);
...
function renderGrid() {
    ...
    CUSTOM_COLS.forEach(c => headerHtml += `<th class="p-3 text-green-400 text-center" onclick="sortData('${c}')">${c}${getSortIndicator(c)}</th>`);
    DEBUG_COLS.forEach(c => headerHtml += `<th class="p-3 text-pink-400 text-center" onclick="sortData('${c}')">${c}${getSortIndicator(c)}</th>`);
    ...
    body.innerHTML = slice.map(d => {
        ...
        if (d.group === 'Test Set' && d.predictedGroup) dispGroup = `<span class="text-purple-400 italic">${d.predictedGroup}</span>`;
        else if (d.meta.bird_species && d.meta.bird_species !== "Unknown") dispGroup += ` <span class="text-[10px] text-slate-500">(${d.meta.bird_species})</span>`;
