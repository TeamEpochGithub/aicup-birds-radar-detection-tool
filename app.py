...
def run_python():
    ...
    try:
        ...
        def serialize(obj):
            ...
            if isinstance(obj, (np.floating, float, Number)): return float(obj)
            if isinstance(obj, Mapping): return {k: serialize(v) for k, v in obj.items()}
            if isinstance(obj, (np.ndarray, Iterable)):
                return [serialize(i) for i in (obj.tolist() if hasattr(obj, 'tolist') else list(obj))]
            try:
                return str(obj)
            except Exception:
                return None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--listen", "-l", default=os.getenv("LISTEN", "127.0.0.1"), help="Host to bind the web server to. Defaults to localhost which means only your machine can access it. Set to 0.0.0.0 to expose the web server outside your machine")
    parser.add_argument("--port", "-p", default=int(os.getenv("PORT", "8000")), type=int, help="Port to bind the web server to. Defaults to port 8000")

    args = parser.parse_args()

    print("\n" + "="*70)
    print("BIRD RADAR LOCAL SERVER")
    print(f"URL: http://{"localhost" if args.listen in ["0.0.0.0", "127.0.0.1", "::1", "::"] else args.listen}:{str(args.port)}/#token={PYTHON_TOKEN}")
