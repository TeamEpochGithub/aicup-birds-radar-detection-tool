...
def write_binary(filename, df, is_test=False):
    with open(filename, "wb") as f:
        ...
        for _, row in df.iterrows():
            ...
            file_buffer.extend(
                struct.pack("<I", to_timestamp(row.get("timestamp_start_radar_utc")))
            )
            file_buffer.extend(
                struct.pack("<I", to_timestamp(row.get("timestamp_end_radar_utc")))
            )
            ...
            if not is_test:
                ...
                try:
                    obs_id = int(float(row.get("observation_id", 0)))
                except:
                    obs_id = 0
                file_buffer.extend(struct.pack("<I", obs_id))

                try:
                    p_obs_id = int(float(row.get("primary_observation_id", 0)))
                except:
                    p_obs_id = 0
                ...
                # 10. N Birds (Uint16)
                try:
                    n_birds = int(float(row.get("n_birds_observed", 1)))
                except:
                    n_birds = 1
